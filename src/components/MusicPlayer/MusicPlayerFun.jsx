  ...
  const volumeContent = () => {
    return (
      <div className='volume-content' onClick={handleSetPlayerMute}>
        {props.player.mute ? <i className='iconfont icon-guanbiyinliang-02'></i> : <i className='iconfont icon-yinliangjian'></i>}
      </div>
    )
  }
      ...
      <div className='music-play-list'>
        ...
        <div className='list-body'>
          {props.playList.map((item, index) => {
            return (
              <div
                className={props.currentSong.index === index ? 'list-body--item list-body--item_current' : 'list-body--item'}
                onClick={() => handleChangeSong(item.songmid, index)}
                key={item.songid}
              >
                <div className='item-cover'>
                  <img src={item.song_cover || defaultCover} alt='' />
                </div>
                <div className='item-infos'>
                  <p className='item-infos--name'>{item.songname}</p>
                  <p className='item-infos--singer'>{handleGetSiner(item.singer)}</p>
                </div>
              </div>
            )
          })}
  ...
  return (
    <div className='music-player-fun'>
      <div className='fun-item fun-item--time'>
        {handleTimestampFormat(props.currentSong.current, 'mm:ss', 's')} / {handleTimestampFormat(props.currentSong.duration, 'mm:ss', 's')}
      </div>
      <div className='fun-item fun-item--volume'>
        <Popover placement='top' title={volumeTitle} content={volumeContent} trigger='click'>
          {props.player.mute ? <i className='iconfont icon-guanbiyinliang-02'></i> : <i className='iconfont icon-yinliangjian'></i>}
        </Popover>
      </div>
      ...
      <div className='fun-item fun-item--list'>
        <i className='iconfont icon-24gf-playlistMusic4' onClick={() => setPlayListShow(true)}></i>
